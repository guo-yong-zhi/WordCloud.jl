...
When you have set `style=:gathering`, you should disable teleporting in `generate!` at the same time, especially for big words. e.g. `generate!(wc, teleport=0.7)`.
"""
...
"""
function fit!(wc, args...; teleport=true, krags...)
    teleporton = teleport isa Union{Function, Number} ? teleport : index(wc, teleport) #Bool <: Number
    if STATEIDS[getstate(wc)] < STATEIDS[:placement!]
        placement!(wc)
    end
    qtrees = [wc.maskqtree, wc.qtrees...]
    ep, nc = train!(qtrees, args...; teleport=teleporton, krags...)
    wc.params[:epoch] += ep
    if nc == 0
